...
- **流式下载**（`event.dart:downloadAndDecryptAttachment`）：支持下载进度回调和取消令牌，大文件不会溢出内存
- **临时文件管理**：`decryptFile` 返回临时文件路径；调用方应在使用后显式 `File(path).delete()`，防止磁盘空间泄漏

**App 层职责**：
- 播放视频后应调用播放器 `dispose()`，防止内存中的 decode frame 泄漏
- 下载的缓存文件（从 `downloadAndDecryptAttachment` 获得的 `MatrixFile.path`）应在使用完毕后删除
